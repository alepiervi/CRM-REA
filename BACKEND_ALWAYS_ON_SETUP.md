...
const getBackendURL = () => {
  const forcedURL = 'https://client-search-fix-3.preview.emergentagent.com';
...
async function keepAlive() {
  await fetch('https://client-search-fix-3.preview.emergentagent.com/api/health');
}
```
...
jobs:
  ping:
    ...
    steps:
      - run: curl https://client-search-fix-3.preview.emergentagent.com/api/health
```
