...
const getBackendURL = () => {
  const forcedURL = 'https://crm-workflow-boost.preview.emergentagent.com';
...
curl https://crm-workflow-boost.preview.emergentagent.com/api/health
...
async function keepAlive() {
  await fetch('https://crm-workflow-boost.preview.emergentagent.com/api/health');
}
```
...
jobs:
  ping:
    ...
    steps:
      - run: curl https://crm-workflow-boost.preview.emergentagent.com/api/health
```
