...
async def create_lead_webhook_get(
    ...
    if should_start_qualification:
        ...
        if final_unit_id:
            ...
            if unit and not unit.get("auto_assign_enabled", True):
                # Auto-assignment disabled - assign directly to the Unit's referente
                logging.info(f"[WEBHOOK GET] Unit {final_unit_id} has auto_assign disabled. Looking for referente...")
                ...
                if referente:
                    ...
                    logging.info(f"[WEBHOOK GET] Lead {lead_obj.id} assigned to referente {referente_name} ({referente_id}) for unit {unit.get('nome')} (auto_assign disabled)")
...
async def create_lead_webhook_post(lead_data: LeadCreate):
    ...
    if should_start_qualification:
        ...
        if unit_id:
            ...
            if unit and not unit.get("auto_assign_enabled", True):
                # Auto-assignment disabled - assign directly to the Unit's referente
                logging.info(f"[WEBHOOK POST] Unit {unit_id} has auto_assign disabled. Looking for referente...")
                ...
                if referente:
                    ...
                    logging.info(f"[WEBHOOK POST] Lead {lead_obj.id} assigned to referente {referente_name} ({referente_id}) for unit {unit.get('nome')} (auto_assign disabled)")
                    
                    # Send email notification to referente
                    asyncio.create_task(notify_agent_new_lead(referente_id, lead_obj.dict()))
                else:
                    logging.warning(f"[WEBHOOK POST] No referente found for unit {unit_id}. Lead {lead_obj.id} will remain unassigned.")
            else:
...
@api_router.post("/leads-cestino/{lead_id}/ripristina")
...
@api_router.delete("/leads-cestino/{lead_id}/elimina-definitivo")
...
async def get_referente_analytics(
    ...
    if current_user.role == UserRole.REFERENTE and current_user.id != referente_id:
        ...
        # Supervisor can view analytics for referenti in their Unit
        referente = await db.users.find_one({"id": referente_id})
        if not referente or referente.get("unit_id") != current_user.unit_id:
            raise HTTPException(status_code=403, detail="Puoi vedere analytics solo dei referenti nella tua Unit")
    elif current_user.role != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Not enough permissions")
